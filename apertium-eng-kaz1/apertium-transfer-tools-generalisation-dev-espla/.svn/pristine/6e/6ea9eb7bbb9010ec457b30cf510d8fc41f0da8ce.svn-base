...
DEFINE_boolean 'only_lexical' false 'Do not generalise over unseen attributes' 'L'
...
if [ "${FLAGS_only_lexical}" == "${FLAGS_TRUE}" ]; then
	...
	NEWGENERATIONALGORITHMFLAG="$NEWGENERATIONALGORITHMFLAG -7"
fi
...
NUMPARTSTHRESHOLDS=`echo "${FLAGS_filtering_thresholds}" | tr '_' ' ' | wc -w`
  ...
  if [ "$NUMPARTSTHRESHOLDS" == "1" ]; then
    ...
    THRESHOLD_START=`echo "${FLAGS_filtering_thresholds}" | cut -f 1 -d '_'`
    THRESHOLD_STEP=`echo "${FLAGS_filtering_thresholds}" | cut -f 2 -d '_'`
    THRESHOLD_END=`echo "${FLAGS_filtering_thresholds}" | cut -f 3 -d '_'`
  fi
  
SEQ_PROPCORRECT_EXTENDED_START=$THRESHOLD_START
SEQ_PROPCORRECT_EXTENDED_STEP=$THRESHOLD_STEP
SEQ_PROPCORRECT_EXTENDED_END=$THRESHOLD_END
...
if [ "${FLAGS_use_retratos_bildic}" == "${FLAGS_TRUE}" ]; then
  cp $HOME/data/input/${PAIR}/retratosdict/$PAIR-$SIZE.bin $TARGET_SOURCES/apertium-$APERTIUM_PAIR/${PAIR}.autobil.shortrestrictions.bin
fi
...
cp $HOME/data/input/$PAIR/$BILPHRASES_ID/shuf${SIZE}l5.tt1ats-f-* $TARGET_SOURCES
cp $HOME/data/input/$PAIR/$BILPHRASES_ID/shuf${SIZE}l5.tt1bilphrases.gz $TARGET_SOURCES
...
SEQ_START=$SEQ_PROPCORRECT_EXTENDED_START
SEQ_STEP=$SEQ_PROPCORRECT_EXTENDED_STEP
SEQ_END=$SEQ_PROPCORRECT_EXTENDED_END
for THRESHOLD in `LC_ALL=C seq $SEQ_START $SEQ_STEP $SEQ_END`; do
	MYATSFILE=$TARGET_SOURCES/shuf${SIZE}l5.tt1ats-f-$THRESHOLD
	bash $TARGET_SOURCES/apertium-transfer-tools-generalisation/rule-generalisation/beamSearchForTT1.sh --target_language $TL --dir $EXPERIMENT_DIR/shuf$SIZE/beamsearch-$FILTERING_NAME/ --sentences $SENTENCES_ARGUMENT $PART_ARGUMENT --python_home $TARGET_SOURCES/Python-2.6.7/bin/ --apertium_data_dir $TARGET_LOCAL/share/apertium --ats_file $MYATSFILE --bilingual_dictionary $TARGET_SOURCES/apertium-$APERTIUM_PAIR/${PAIR}.autobil.shortrestrictions.bin --result_suffix "-$THRESHOLD" --tag_groups_seqs_suffix "_$PAIR" --step "1"
...
#get beam search results
cp $HOME/results/experiments-linear-l5-${PAIR}/$BILPHRASES_ID/shuf$SIZE/beamsearch-${FILTERING_NAME}-*-*.tar.gz $EXPERIMENT_DIR/shuf$SIZE/

cd $EXPERIMENT_DIR/shuf$SIZE/
for MYFILE in beamsearch-${FILTERING_NAME}-*-*.tar.gz ; do
	  tar xzf $MYFILE
done

#copy necessary resources to maximisation score directory
NUMBERS=`ls $EXPERIMENT_DIR/shuf$SIZE/beamsearch-$FILTERING_NAME/ | grep "^scores-" | grep 'gz$' | awk -F"-" '{print $2}' | LC_ALL=C sort | uniq`
TOTALNUMBERS=`echo "$NUMBERS" | wc -l`
for NUMBER in $NUMBERS ; do
	cat $EXPERIMENT_DIR/shuf$SIZE/beamsearch-$FILTERING_NAME/sentences.withtt1restrictions-$NUMBER-$TOTALNUMBERS >> $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/sentences.withtt1restrictions
done
...
for THRESHOLD in `LC_ALL=C seq $SEQ_START $SEQ_STEP $SEQ_END`; do
	...
	if [ "$PART" == "" -o "$PART" == "$NUMSTEP" ] ; then

	MYATSFILE=$TARGET_SOURCES/shuf${SIZE}l5.tt1ats-f-$THRESHOLD
	
	#get scores and supersegments
	for NUMBER in $NUMBERS ; do
		zcat $EXPERIMENT_DIR/shuf$SIZE/beamsearch-$FILTERING_NAME/scores-$NUMBER-$TOTALNUMBERS-$THRESHOLD.gz >> $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/scores-$THRESHOLD
		zcat $EXPERIMENT_DIR/shuf$SIZE/beamsearch-$FILTERING_NAME/supersegments-$NUMBER-$TOTALNUMBERS-$THRESHOLD.gz >> $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/supersegments-$THRESHOLD
	done
	
	gzip $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/scores-$THRESHOLD
	gzip $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/supersegments-$THRESHOLD
	cp $EXPERIMENT_DIR/shuf$SIZE/beamsearch-$FILTERING_NAME/finalboxesindex-$THRESHOLD $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/
	
	# super debug
	echo "COMMAND: bash $TARGET_SOURCES/apertium-transfer-tools-generalisation/rule-generalisation/beamSearchForTT1.sh --target_language $TL --dir $EXPERIMENT_DIR/shuf$SIZE/maximise-score-$FILTERING_NAME/  $PART_ARGUMENT --python_home $TARGET_SOURCES/Python-2.6.7/bin/ --apertium_data_dir $TARGET_LOCAL/share/apertium --ats_file $MYATSFILE --bilingual_dictionary $TARGET_SOURCES/apertium-$APERTIUM_PAIR/${PAIR}.autobil.shortrestrictions.bin --result_suffix \"-$THRESHOLD\" --tag_groups_seqs_suffix \"_$PAIR\" --step \"2\""
...
#get maximisation
cp $HOME/results/experiments-linear-l5-${PAIR}/$BILPHRASES_ID/shuf$SIZE/maximise-score-$FILTERING_NAME-*.tar.gz $EXPERIMENT_DIR/shuf$SIZE/

cd $EXPERIMENT_DIR/shuf$SIZE/
for MYFILE in maximise-score-$FILTERING_NAME-*.tar.gz ; do
	  tar xzf $MYFILE
done

MYBILPHRASESFILE=$TARGET_SOURCES/shuf${SIZE}l5.tt1bilphrases.gz
...
for THRESHOLD in `LC_ALL=C seq $SEQ_START $SEQ_STEP $SEQ_END`; do
	...
	mkdir -p $EXPERIMENT_DIR/shuf$SIZE/remove-redundant-$FILTERING_NAME-$THRESHOLD
	MYATSFILE=$TARGET_SOURCES/shuf${SIZE}l5.tt1ats-f-$THRESHOLD
	
	bash $TARGET_SOURCES/apertium-transfer-tools-generalisation/rule-generalisation/removeRedundantRulesForTT1.sh --source_language $SL --target_language $TL --dir $EXPERIMENT_DIR/shuf$SIZE/remove-redundant-$FILTERING_NAME-$THRESHOLD --python_home $TARGET_SOURCES/Python-2.6.7/bin/ --ats_file $MYATSFILE --bilingual_phrases_file $MYBILPHRASESFILE --bilingual_dictionary $TARGET_SOURCES/apertium-$APERTIUM_PAIR/${PAIR}.autobil.shortrestrictions.bin --apertium_prefix $APERTIUM_PREFIX --tag_groups_seqs_suffix "_$PAIR" --finalboxesindex_beamsearch  $EXPERIMENT_DIR/shuf$SIZE/maximise-score-default/finalboxesindex-$THRESHOLD --boxesid_beamsearch $EXPERIMENT_DIR/shuf$SIZE/maximise-score-default/rulesid-$THRESHOLD
	
	tar czf remove-redundant-$FILTERING_NAME-$THRESHOLD.tar.gz remove-redundant-$FILTERING_NAME-$THRESHOLD
	cp remove-redundant-$FILTERING_NAME-$THRESHOLD.tar.gz $HOME/results/experiments-linear-l5-${PAIR}/$BILPHRASES_ID/shuf$SIZE
