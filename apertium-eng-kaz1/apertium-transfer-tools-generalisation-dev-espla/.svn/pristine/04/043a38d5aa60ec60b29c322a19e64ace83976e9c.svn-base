...
if [ "${FLAGS_ats_file}" != "" ]; then
	...
	ATS_FILE=${FLAGS_dir}/ats${FLAGS_result_infix}${FLAGS_ats_suffix}
	rm -f ${ATS_FILE%.gz}
	...
	if [ "${FLAGS_alternative_ats_filtering_dir}" != "" ]; then
		ALTATSSUFFIX="-alt1"
		ALT_ATS_FILE="${FLAGS_dir}/ats${FLAGS_result_infix}$ALTATSSUFFIX${FLAGS_ats_suffix}"
...
cat ${FLAGS_final_boxes_index} | python $CURDIR/addStandardBoxDescToOnlyLexical.py > ${FLAGS_dir}/finalboxesindex.plusstd
cut -f 3 ${FLAGS_dir}/finalboxesindex.plusstd | LC_ALL=C sort | LC_ALL=C uniq | awk '{printf "%d\t%s\n", NR, $0}' > ${FLAGS_dir}/stdfinalboxesindex
LC_ALL=C sort -t'	' -k3,3 ${FLAGS_dir}/finalboxesindex.plusstd | join -t'	' -1 3 -2 2 - ${FLAGS_dir}/stdfinalboxesindex | awk -F'\t' '{ print $2 "\t"  $4 }' > ${FLAGS_dir}/mappingsfinalboxestostdfinalboxes

BOXESFLAG="--final_boxes_index  ${FLAGS_dir}/stdfinalboxesindex --minimum_covered_words --allow_incompatible_rules"
BOXESFLAGSEC="--final_boxes_index  ${FLAGS_dir}/stdfinalboxesindex"
...
zcat ${FLAGS_sentences} | ${FLAGS_python_home}python $CURDIR/beamSearch.py --target_language ${FLAGS_target_language} --alignment_templates $ATS_FILE $ALT_ATS_FLAG --tag_groups_file_name $CURDIR/taggroups${FLAGS_tag_groups_seqs_suffix} --tag_sequences_file_name $CURDIR/tagsequences${FLAGS_tag_groups_seqs_suffix} --apertium_data_dir "${FLAGS_apertium_data_dir}" $BOXESFLAG $DEBUG_FLAG --beam_size 2000 $RBPE_FLAG 2> ${FLAGS_dir}/scores${FLAGS_result_infix}${FLAGS_ats_suffix}-debug | awk -F"[|][|][|]"  '{ print $1; }' | gzip > ${FLAGS_dir}/scores${FLAGS_result_infix}${FLAGS_ats_suffix}

if [ "$ALT_ATS_FILE" == "" ]; then
	zcat ${FLAGS_dir}/scores${FLAGS_result_infix}${FLAGS_ats_suffix} | ${FLAGS_python_home}python $CURDIR/computeSupersetsOfKeySegments.py --tag_groups_file_name $CURDIR/taggroups${FLAGS_tag_groups_seqs_suffix} --tag_sequences_file_name $CURDIR/tagsequences${FLAGS_tag_groups_seqs_suffix} $BOXESFLAGSEC --alignment_templates $ATS_FILE --sentences ${FLAGS_sentences} --target_language ${FLAGS_target_language} --apertium_data_dir "${FLAGS_apertium_data_dir}" $RBPE_FLAG 2>${FLAGS_dir}/supersegments${FLAGS_result_infix}${FLAGS_ats_suffix}.debug | gzip  > ${FLAGS_dir}/supersegments${FLAGS_result_infix}${FLAGS_ats_suffix}
fi

popd

if [ "${FLAGS_ats_file}" == "" ]; then
	cp ${FLAGS_dir}/ats${FLAGS_result_infix}${FLAGS_ats_suffix} ${FLAGS_dir}/ats${FLAGS_ats_suffix}
fi
