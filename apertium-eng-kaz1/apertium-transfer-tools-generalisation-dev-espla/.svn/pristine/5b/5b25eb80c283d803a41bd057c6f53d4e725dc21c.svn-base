...
if [ -d test-ttbeam-$PAIR ]; then

pushd test-ttbeam-$PAIR > /dev/null
...
cat testtest.${SL}-${TL}.source > $TMPSRC
cat testtest.${SL}-${TL}.translation_tt1beam > $TMPTRG
cat testtest.${SL}-${TL}.reference > $TMPREF
