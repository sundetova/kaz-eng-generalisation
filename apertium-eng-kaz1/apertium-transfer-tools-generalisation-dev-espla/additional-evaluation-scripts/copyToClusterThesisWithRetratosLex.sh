		...
		for BILDICVARIANT in "dontallowincompwithbildic," ; do
			...
			for EXTREMESVARIANT in "endsaligned,ends_aligned" ; do
				...
				for PUNCTUATIONVARIANT in "dontallowpunct," ; do
					...
					bash $ROOTDIR/rule-generalisation/copyToCluster.sh "$OUTPURDIR/work-$APERTIUMPAIR-myextractionextendedcorpuswithretratosdict-$CORPUSNAME-thesis-$EXTREMESNAME-$BILDCNAME-$PUNCTNAME/$SIZE/bilingualphrases" "$SL-$TL" "myextractionextendedcorpuswithretratosdict-$CORPUSNAME-thesis-$EXTREMESNAME-$BILDCNAME-$PUNCTNAME" "$SIZE" 
