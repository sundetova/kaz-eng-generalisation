...
DEFINE_string 'corpus' '/home/apertium/apertium-testing/apertium-eng-kaz/texts/texts2' 'prefix of files containing parallel corpus (suffixes are .SL and .TL)' 'c'
...
DEFINE_string 'apertium_prefix' '/home/apertium/.local' 'Prefix where Apertium was installed (default: /usr/local/)' 'u'
######################
# (optional)
...
if [ "${FLAGS_transfer_tools_1}" == "${FLAGS_TRUE}" ]; then
  ...
  if [ "${FLAGS_only_extract_bilingual_phrases}" != "${FLAGS_TRUE}" ]; then
    ...
    APERTIUMPREFIXDATADIR="/work/espla/kazakh/Projects/apertium-eng-kaz"
