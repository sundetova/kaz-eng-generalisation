...
INTERNALTL="${SL}_rbpe"
POSTTRANSFERRULES=$FULLCURDIR/../phrase-extraction/transfer-tools-scripts/apertium-${TL}-$SL.posttransfer.ptx
fi
...
if $ONLY_GENERATE_XML ; then
  ...
  bash $CURDIR/createModeNoRules.sh "$ORIGINALAPERTIUMMODE" "$TRANSFERTOOLSPATH" $FULLCURDIR/empty-rules-for-translating.t1x $EXPQUERIESDIR/rules/empty-rules-for-translating.t1x.bin $POSTTRANSFERRULES $BINBIDICTIONARY "$LEXICAL_SELECTION_COMMAND" > $EXPQUERIESDIR/modes/${SL}-${INTERNALTL}_norules.mode
  cat $EXPQUERIESDIR/evaluation/source | bash $CURDIR/translate_apertium.sh "$APERTIUMPATH" ${SL}-${INTERNALTL}_norules join "" $EXPQUERIESDIR > $EXPQUERIESDIR/evaluation/translation_norules
  ...
  bash $CURDIR/createModeWithLearnedRules.sh $ORIGINALAPERTIUMMODE "$TRANSFERTOOLSPATH" $EXPQUERIESDIR/rules/rules $POSTTRANSFERRULES $BINBIDICTIONARY "${PYTHONHOME}python $FULLCURDIR/removeDebugInfoFromTransfer.py" "$LEXICAL_SELECTION_COMMAND" "" "$ALT_MODE_FOR_RBPE" > $EXPQUERIESDIR/modes/${SL}-${INTERNALTL}_learned.mode 
  cat $EXPQUERIESDIR/evaluation/source | bash $CURDIR/translate_apertium.sh "$APERTIUMPATH" ${SL}-${INTERNALTL}_learned join "" $EXPQUERIESDIR > $EXPQUERIESDIR/evaluation/translation_learnedrules 2> $EXPQUERIESDIR/evaluation/used_rules

  #translate and not remove unknown mark
  cat $EXPQUERIESDIR/evaluation/source | bash $CURDIR/translate_apertium.sh "$APERTIUMPATH" ${SL}-${INTERNALTL}_learned join "" $EXPQUERIESDIR --show_unknown > $EXPQUERIESDIR/evaluation/translation_learnedrules_withunknown 2> /dev/null
  ...
  echo "${TRANSFERTOOLSPATH}apertium-gen-transfer-from-aligment-templates --input $EXPQUERIESDIR/rules/alignmentTemplates.txt --attributes $TAGGROUPS --generalise --nodoublecheckrestrictions --usediscardrule $RICHATSFLAG $RBPE_GENTRANSFER_FLAG | ${PYTHONHOME}python $CURDIR/addDebugInfoToTransferRules-debug.py > $EXPQUERIESDIR/rules/rules-extradebug.xml" > $TMPFILE2
  bash $TMPFILE2
  ...
  bash $CURDIR/createModeWithLearnedRules.sh $ORIGINALAPERTIUMMODE "$TRANSFERTOOLSPATH" $EXPQUERIESDIR/rules/rules-extradebug $POSTTRANSFERRULES $BINBIDICTIONARY "${PYTHONHOME}python $FULLCURDIR/removeDebugInfoFromTransfer.py moredebug" "$LEXICAL_SELECTION_COMMAND" "" "$ALT_MODE_FOR_RBPE" > $EXPQUERIESDIR/modes/${SL}-${INTERNALTL}_extradebug.mode 
  cat $EXPQUERIESDIR/evaluation/source | bash $CURDIR/translate_apertium.sh $APERTIUMPATH ${SL}-${INTERNALTL}_extradebug join "" $EXPQUERIESDIR > $EXPQUERIESDIR/evaluation/output-extradebug 2> $EXPQUERIESDIR/evaluation/debug-extradebug
  ...
  if [ -z "$AUTOGEN" ]; then
    AUTOGEN=`cat $EXPQUERIESDIR/modes/${SL}-${INTERNALTL}_norules.mode | tr '|' '\n' | grep -F "autogen.htfs" | awk -F '$' '{ print $2;}'|  sed 's_1 __'`;
    #check sed
    cat $EXPQUERIESDIR/evaluation/debug-extradebug-clean |  grep -F "TARGET:" | sed 's_^TARGET: __' | sed 's_^$_EMPTY_' | hfst-proc -g $AUTOGEN | sed 's_\(.*\)_\n\1\n_' > $EXPQUERIESDIR/evaluation/debug-extradebug-clean-tlsurface
  else
