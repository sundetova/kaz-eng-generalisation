...
if [ "${FLAGS_beam}" != "" ]; then
	...
	if [ "${FLAGS_final_boxes_index}" != "" ]; then
	  ...
	  if [ "${FLAGS_only_lexical}" == "${FLAGS_TRUE}" ]; then
	    BEAM_FLAG="$BEAM_FLAG --final_boxes_index ${FLAGS_beam_search_dir}/stdfinalboxesindex"
	  else
...
for number in $NUMBERS; do
	...
	NUMLOCALSCORES=`zcat "${FLAGS_beam_search_dir}/supersegments-${number}-${TOTALNUMBERS}${FLAGS_ats_suffix}" | wc -l`

	zcat "${FLAGS_beam_search_dir}/scores-${number}-${TOTALNUMBERS}${FLAGS_ats_suffix}" | head -n $NUMLOCALSCORES >> ${SCORES_FILE%.gz}
	...
	zcat "${FLAGS_beam_search_dir}/supersegments-${number}-${TOTALNUMBERS}${FLAGS_ats_suffix}" | head -n $NUMLOCALSCORES >> ${ALL_SUPERSEGMENTS_FILE%.gz}
	...
	zcat "${FLAGS_beam_search_dir}/sentences.${number}-${TOTALNUMBERS}.gz" | head -n $NUMLOCALSCORES >> ${ALL_SENTENCES_FILE%.gz}
done
gzip ${SCORES_FILE%.gz}
