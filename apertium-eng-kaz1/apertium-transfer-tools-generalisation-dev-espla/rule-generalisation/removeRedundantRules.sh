...
do
      ...
      for ITERATIONVAR in $ITERATIONSDIRS ; do
      	  ...
      	  if [ "$USE_CORPUS" == "false" -o  "$ITERATIONVAR" == "1-ref" -o "$ITERATIONVARCONTAINSREF" == "0" ] ; then
			  ...
			  TMPFILE1=`mktemp`
			  TMPFILE2=`mktemp`
			  echo "${TRANSFERTOOLSPATH}apertium-gen-transfer-from-aligment-templates --input $EXPQUERIESDIR/rules/alignmentTemplates.txt $GENTRANSFERFLAGS --novarsdetermined --usediscardrule $RICHATSFLAG $RBPE_FLAG_FOR_GEN_TRANSFER 2> $TMPFILE2 | ${PYTHONHOME}python $CURDIR/addDebugInfoToTransferRules.py > $EXPQUERIESDIR/rules/rules.xml" > $TMPFILE1
			  bash $TMPFILE1
			  rm $TMPFILE1
			  #output log
			  cat $TMPFILE2
		  ...
		  if $USE_CORPUS ; then
			...
			if [ "$ITERATIONVAR" == "1-ref" -o "$ITERATIONVARCONTAINSREF" == "0" ] ; then
			 ...
			 cp $MYDIR/translations_$PREVITERATION/wer.best $MYDIR/translations_${ITERATION}/wer.${ITERATIONVAR}
			  ...
			  #Get bilphrases
			  zcat $BILPHRASES_DIR/$BOXNUMBER$DEBUGINFOFILESUFFIX | tac | sed -e '/^BILINGUAL_PHRASES$/,$d' | tac | sed -e '/^END_BILINGUAL_PHRASES$/,$d' | ${PYTHONHOME}python $CURDIR/filterBilphrasesMatchingAT.py --tag_groups_file_name $CURDIR/taggroups$TAGSEQUENCESANDGROUPSSUFFIX --alignment_template "$AT" $FILTERBILPHRASFLAGS $RICHATSFLAG | cut -f 1,2,3,4 -d '|' | sed 's:<empty_tag_[^>]*>::g' | sed -r 's_> ([^|])_>$ ^\1_g' |  sed -r 's_> [|]_>$ |_g' | sed -r 's_[|] ([^^])_| ^\1_' |  sed -r 's_[|] ([^^])_| ^\1_' | gzip > $MYDIR/bilingualPhrases.${ITERATION}.gz
