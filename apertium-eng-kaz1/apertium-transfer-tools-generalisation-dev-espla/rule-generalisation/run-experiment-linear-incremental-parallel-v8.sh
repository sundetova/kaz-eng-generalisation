...
if [ "$STEP" == "1" -o  "$STEP" == "" ]; then
	...
	zcat $INPUTFILE $ALT_INPUTFILE  | ${PYTHONHOME}python $CURDIR/addPosAndRestrictionsStr.py $ONLYLEXICALGENFLAG --tag_groups_file_name $CURDIR/taggroups$TAGSEQUENCESANDGROUPSSUFFIX --tag_sequences_file_name $CURDIR/tagsequences$TAGSEQUENCESANDGROUPSSUFFIX $RICHATSFLAGFORADD | sed 's_ |_|_' | LC_ALL=C sort | ${PYTHONHOME}python $CURDIR/spreadBilphrases.py  > $BOXESINDEX

	zcat $INPUTFILE | ${PYTHONHOME}python $CURDIR/addPosAndRestrictionsStr.py $ONLYLEXICALGENFLAG --tag_groups_file_name $CURDIR/taggroups$TAGSEQUENCESANDGROUPSSUFFIX --tag_sequences_file_name $CURDIR/tagsequences$TAGSEQUENCESANDGROUPSSUFFIX $RICHATSFLAGFORADD | sed 's_ |_|_' | LC_ALL=C sort | ${PYTHONHOME}python $CURDIR/spreadBilphrases.py --dir $TRUEWORKDIR/generalisation --dict $BOXESINDEX

	if [ "$ALT_INPUTFILE" != "" ]; then
		zcat  $ALT_INPUTFILE  | ${PYTHONHOME}python $CURDIR/addPosAndRestrictionsStr.py  $ONLYLEXICALGENFLAG--tag_groups_file_name $CURDIR/taggroups$TAGSEQUENCESANDGROUPSSUFFIX --tag_sequences_file_name $CURDIR/tagsequences$TAGSEQUENCESANDGROUPSSUFFIX $RICHATSFLAGFORADD | sed 's_ |_|_' | LC_ALL=C sort | ${PYTHONHOME}python $CURDIR/spreadBilphrases.py --dir $TRUEWORKDIR/generalisation-alt1 --dict $BOXESINDEX
	fi
...
if [ "$FIRSTMINIMISELEMMAS" == "" ]; then
  ...
  for file in $TRUEWORKDIR/generalisation/boxesofnewdata.sorted.bylength.split* ; do 
	  ...
	  parallel $PARALLELONLYONE -i  bash -c "INDEX=\`echo \"{}\"\`;  if [ \"\`expr \$INDEX % $NUM_PARTS\`\" == \"$CHOSEN_PART_MINUS_ONE\" ] ; then  zcat \"$GENERALISATION_DIR/newbilphrases/\$INDEX.bilphrases.gz\" | ${PYTHONHOME}python $CURDIR/generateMultipleATsFromBilphrases.py --tag_groups_file_name $CURDIR/taggroups$TAGSEQUENCESANDGROUPSSUFFIX --tag_sequences_file_name $CURDIR/tagsequences$TAGSEQUENCESANDGROUPSSUFFIX $RICHATSFLAG $RICHATSFLAGONLYGEN $POWERSETFEATURESFLAG $DIFFERENTRESTRICTIONSFLAG $UNLEXICALISEUNALIGNEDSLFLAG $GENATTRIBUTESLIKEINPAPER $ONLYLEXICALGENFLAG $RBPEFLAG 2> \"$GENERALISATION_DIR/debug/debug-generalisation-\$INDEX\" | gzip > $GENERALISATION_DIR/ats/\$INDEX.ats.gz; gzip \"$GENERALISATION_DIR/debug/debug-generalisation-\$INDEX\"; fi;" -- `cat $file`
	  
	  if [ "$ALT_INPUTFILE" != "" ]; then
		GENERALISATION_DIR=$TRUEWORKDIR/generalisation-alt1
		parallel $PARALLELONLYONE -i  bash -c "INDEX=\`echo \"{}\" | cut -f 2\`; NUMLINE=\`echo \"{}\" | cut -f 1\`;  if [ \"\`expr \$NUMLINE % $NUM_PARTS\`\" == \"$CHOSEN_PART_MINUS_ONE\" ] ; then  zcat \"$GENERALISATION_DIR/newbilphrases/\$INDEX.bilphrases.gz\" | ${PYTHONHOME}python $CURDIR/generateMultipleATsFromBilphrases.py --tag_groups_file_name $CURDIR/taggroups$TAGSEQUENCESANDGROUPSSUFFIX --tag_sequences_file_name $CURDIR/tagsequences$TAGSEQUENCESANDGROUPSSUFFIX $RICHATSFLAG $RICHATSFLAGONLYGEN $POWERSETFEATURESFLAG $DIFFERENTRESTRICTIONSFLAG $UNLEXICALISEUNALIGNEDSLFLAG $GENATTRIBUTESLIKEINPAPER $ONLYLEXICALGENFLAG $RBPEFLAG 2> \"$GENERALISATION_DIR/debug/debug-generalisation-\$INDEX\" | gzip > $GENERALISATION_DIR/ats/\$INDEX.ats.gz; gzip \"$GENERALISATION_DIR/debug/debug-generalisation-\$INDEX\"; fi;" -- `cat $file | awk '{printf "%d\t%s\n", NR, $0}'`
	  fi
  done
